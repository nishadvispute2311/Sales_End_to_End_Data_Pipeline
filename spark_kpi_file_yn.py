...
# Output path
output_dir = "yn_processed_kpis"
os.makedirs(output_dir, exist_ok=True)

# Load CSVs
# yn_sales_project_final/csv_consumed_data_kafka/
cust_df = spark.read.csv('csv_consumed_data_kafka/yn-customers-0812_data.csv', header=True, inferSchema=True)
product_df = spark.read.csv('csv_consumed_data_kafka/yn-products-0812_data.csv', header=True, inferSchema=True)
orders_df = spark.read.csv('csv_consumed_data_kafka/yn-orders-0812_data.csv', header=True, inferSchema=True)
order_items_df = spark.read.csv('csv_consumed_data_kafka/yn-order_items-0812_data.csv', header=True, inferSchema=True)
...
kpi_df.coalesce(1).write.option("header",True).mode("overwrite").csv("yn_processed_kpis/static_summary_kpis")
...
ranked_ltv.coalesce(1).write.mode("overwrite").option("header", True).csv(f"{output_dir}/yn_customer_ltv")
...
# docker run -d --name kpi-cron -v D:/kafka-docker-project/yn_sales_project_final:/app spark-kpi-cron -- Final run cron command
